...
\lecturechapter{Dienstag}{10. Mär}{10. März 2025}{Gödelscher Vollständigkeitssatz und Konsistenz}
...
In einer Übung wurde gezeigt, dass für alle Formeln $\psi$ und $\chi$ gilt:
...
\begin{math-stroke}[Existenz eines Modells impliziert Konsistenz]
...
Anordnungssystem $T$, formal $\sigma$.
...
\textbf{Bsp:} $\sigma \equiv \forall x \forall y (x \cdot y = e \rightarrow y \cdot x = e)$.
...
Okay, wir beweisen das jetzt wie folgt: Da wir in einem Modell der Gruppenaxiome arbeiten, wissen wir nach Axiom $\text{GT}_2$, dass ein Linksinverses existiert. Das heißt, zu jedem $x \in G$ gibt es ein $x' \in G$ mit $x' \cdot x = e$. Da $x'$ selbst ein Element der Gruppe ist, besitzt auch $x'$ ein Linksinverses $x'' \in G$ mit $x'' \cdot x' = e$.
...
\begin{math-stroke}[Grenzen der Axiomatisierung]
...
\inlinemetanote{Der Dozent schreibt die Überschrift \qt{Die Zermelo-Fraenkel Axiome} an die Tafel.}
...
\begin{itemize}
    ...
    \item \textbf{Fraenkel (1921):} 2 Axiome
\end{itemize}
...
\textbf{Eindeutigkeit der leeren Menge:}
Das Extensionalitätsaxiom impliziert direkt, dass die leere Menge eindeutig ist. Falls $x_1$ und $x_2$ beide leere Mengen sind, so gilt:
...
\begin{math-stroke}[Eigenschaften von Paarmengen und geordneten Paaren]
...
\begin{itemize}
    \item \textbf{Korrektheitssatz:} Wenn eine Formel $\sigma$ aus einer Theorie $T$ formal beweisbar ist ($T \vdash \sigma$), dann ist sie in jedem Modell von $T$ wahr ($T \models \sigma$).
