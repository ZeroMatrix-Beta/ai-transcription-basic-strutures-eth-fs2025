...
\lecturechapter{Dienstag}{17. Mär}{17. März 2025}{Zermelo-Fraenkel-Mengenlehre (ZFC)}
...
\section{Zermelo-Fraenkel-Mengenlehre (ZFC)}

\subsection{Die ersten Axiome der Mengenlehre}

\begin{math-stroke}[Axiome 0 bis 3 der Mengenlehre]
...
\textbf{Wir definieren die Vereinigungsfunktion:}
\[
...
\textbf{Wir definieren die binäre Vereinigungsfunktion:}
Für zwei Mengen $x$ und $y$ definieren wir die Vereinigung $x \cup y$ durch:
\[
x \cup y := \bigcup \{x, y\}
\]
\begin{explanation-of-steps}
Nach dem Paarmengenaxiom existiert die Paarmenge $\{x, y\}$. Wenden wir das Vereinigungsaxiom auf diese Paarmenge an, erhalten wir genau die Menge aller Elemente, die in $x$ oder in $y$ liegen. Die Eindeutigkeit dieser Menge folgt direkt aus dem Extensionalitätsaxiom.
...
Gut. Man muss sagen, dass das hier natürlich existiert und eindeutig ist; deswegen können wir es so schreiben, auch wieder mit dem Extensionalitätsaxiom. Okay, dann: Wo sind wir? Drei. Dann kommt jetzt... Ah nein, es geht noch weiter. Wir können jetzt nämlich damit... was wir jetzt machen wollen, ist, die ganzen Zahlen zu definieren. Oder so etwas, was sich herausstellt, ist dann ein Modell für die Peano-Arithmetik. Und dazu definieren wir, wenn wir eine Menge $x$ haben, die Menge $x + 1$ als die Menge, die aus $x$ vereinigt mit der Menge besteht, die nur die Menge $x$ enthält. Also wir nehmen die Menge $x$ zusammen mit der Menge, die nur ein Element enthält, nämlich die Menge $x$. Das gibt uns eine neue Menge, und diese Menge bezeichnen wir mit $x + 1$. Okay, das dürfen wir machen.
...
\textbf{Wir definieren den Nachfolger einer Menge:}
Für eine Menge $x$ definieren wir den Nachfolger $x + 1$ (oft auch als $S(x)$ bezeichnet) durch:
\[
x + 1 := x \cup \{x\}
\]

\textbf{Definition der natürlichen Zahlen (von Neumannsches Modell):}
\begin{align*}
0 &:= \emptyset \\
...
n = \{0, 1, \dots, n-1\}
\]
Die intuitive Idee des \qt{Zählens} und der Pünktchenschreibweise ($\dots$) ist mathematisch noch nicht rigoros, da wir die Existenz der Gesamtheit aller natürlichen Zahlen als fertige Menge noch nicht bewiesen haben. Dazu benötigen wir ein eigenes Axiom: das Unendlichkeitsaxiom.
\end{explanation-of-steps}
\end{math-stroke}
...
\textbf{Formale Definition als einstelliges Prädikat:}
Wir definieren das einstellige Prädikat $\operatorname{ind}(x)$ durch:
\[
...
Die durch das Aussonderungsaxiom eindeutig bestimmte Teilmenge $y \subseteq x$ schreiben wir als:
\[
...
\textbf{Wir definieren das binäre Funktionssymbol Durchschnitt ($\cap$):}
Für zwei Mengen $x_0$ und $x_1$ definieren wir den Durchschnitt $x_0 \cap x_1$ durch:
\[
x_0 \cap x_1 = y :\iff y = \{ z \in x_1 \mid z \in x_0 \}
\]
\begin{explanation-of-steps}
Nach dem Aussonderungsaxiom existiert diese Menge, da wir die Bedingung $\varphi(z) \equiv z \in x_0$ auf die bereits existierende Menge $x_1$ anwenden. Hierbei fungiert $x_0$ als Parameter der Formel.
\end{explanation-of-steps}
\end{math-stroke}
...
\textbf{Wir definieren den allgemeinen Durchschnitt ($\bigcap$):}
Sei $\varphi(u) \equiv \forall z \, (z \in x \rightarrow u \in z)$. Für eine nichtleere Menge von Mengen $x$ definieren wir den Durchschnitt $\bigcap x$ durch:
\[
...
\textbf{Wir definieren die Menge $\omega$:}
\[
...
\begin{math-stroke}[Eigenschaften von \texorpdfstring{$\omega$}{omega}]
\begin{claim}
...
\begin{math-stroke}[Die natürlichen Zahlen in \texorpdfstring{$\omega$}{omega}]
...
\begin{math-stroke}[Definition des kartesischen Produkts]
...
\begin{math-stroke}[Geordnete Paare und Potenzmengenbeziehung]
\textbf{Kuratowski-Definition geordneter Paare:}
\[
\langle x, y \rangle := \bigl\{ \{x\}, \{x, y\} \bigr\}
\]

\textbf{Existenz und Schranke des kartesischen Produkts:}
Das kartesische Produkt $A \times B$ existiert als Menge und es gilt:
\[
...
Daraus folgt, dass das geordnete Paar $\langle x, y \rangle = \{\{x\}, \{x, y\}\}$ eine Teilmenge der Potenzmenge ist, also ein Element der doppelten Potenzmenge: $\langle x, y \rangle \in \mathcal{P}\bigl(\mathcal{P}(A \cup B)\bigr)$.
\end{explanation-of-steps}
\end{math-stroke}
...
\begin{math-stroke}[Definition von Funktionen]
...
\textbf{Formale Definition der eindeutigen Existenz ($\exists!$):}
Der Quantor der eindeutigen Existenz $\exists! y \, \varphi(y)$ ist eine Abkürzung für:
\[
...
\begin{math-stroke}[Funktionsnotation und mehrstellige Funktionen]
\textbf{Notationskonventionen:}
Für eine Funktion $f \in {}^A B$ schreiben wir:
\[
f \colon A \to B
\]
...
\begin{math-stroke}[Injektivität]
...
\begin{math-stroke}[Endlichkeit und Abzählbarkeit]
...
\begin{math-stroke}[Relationen und lineare Ordnungen]
...
\begin{math-stroke}[Wohlordnungen]
...
\section{ZFC-Axiome von Fraenkel}

\subsection{Das Ersetzungsaxiom}

\begin{math-stroke}[Das Ersetzungsaxiom (Axiom 7)]
...
\textbf{Klassenfunktionen:}
Eine Formel $\varphi(x, y)$, die die Bedingung $\forall x \exists! y \, \varphi(x, y)$ erfüllt, definiert eine \newterm{Klassenfunktion} $F$. Das Bild einer Menge $A$ unter $F$ bezeichnen wir mit:
\[
...
\subsection{Das Fundierungsaxiom}

\begin{math-stroke}[Das Fundierungsaxiom (Axiom 8)]
