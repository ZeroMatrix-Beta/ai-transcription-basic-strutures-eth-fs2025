...
\lecturechapter{Dienstag}{24. Mär}{24. März 2026}{Zermelo-Fraenkel-Axiome und Dedekindsche Schnitte}
...
\section{Dedekindsche Schnitte und die Konstruktion der reellen Zahlen}

\begin{math-stroke}[Von den rationalen zu den reellen Zahlen]
Um die Lücken in den rationalen Zahlen $\mathbb{Q}$ (wie z.\,B. $\sqrt{2} \notin \mathbb{Q}$) zu schließen und das Vollständigkeitsaxiom \eqref{eq:vollstaendigkeit} zu erfüllen, konstruiert man die reellen Zahlen $\mathbb{R}$ mittels Dedekindscher Schnitte als Teilmengen von $\mathbb{Q}$.

\begin{definition}[Dedekindscher Schnitt und reelle Zahlen]\label{def:reelle-zahlen}
Ein \textbf{Dedekindscher Schnitt} ist eine Teilmenge $D \subseteq \mathbb{Q}$, welche die folgenden drei Eigenschaften erfüllt:
\begin{enumerate}[label=\textbf{(\roman*)}]
    \item \textbf{Nicht-Trivialität:} $D \neq \emptyset$ und $D \neq \mathbb{Q}$.
    \item \textbf{Abgeschlossenheit nach unten:} Für alle $p \in D$ und alle $q \in \mathbb{Q}$ mit $q < p$ gilt auch $q \in D$.
    \item \textbf{Kein größtes Element:} Zu jedem $p \in D$ existiert ein $r \in D$ mit $p < r$.
\end{enumerate}
Die Menge der reellen Zahlen $\mathbb{R}$ wird als die Menge aller Dedekindschen Schnitte definiert:
\[
\mathbb{R} \coloneq \{ D \in \mathcal{P}(\mathbb{Q}) \mid D \text{ ist ein Dedekindscher Schnitt} \} \subseteq \mathcal{P}(\mathbb{Q}).
\]
Jede reelle Zahl $x \in \mathbb{R}$ entspricht somit der Menge aller rationalen Zahlen, die strikt kleiner als $x$ sind ($D_x = \{ q \in \mathbb{Q} \mid q < x \}$).
\end{definition}
\end{math-stroke}

\begin{nice-box}[Zusammenfassung der Schlüsselkonzepte]
In diesem Vorlesungsabschnitt haben wir drei fundamentale Konzepte behandelt:
\begin{itemize}
    ...
    \item \textbf{Dedekindsche Schnitte (Definition \ref{def:reelle-zahlen}):} Die Konstruktion von $\mathbb{R}$ als Teilmenge von $\mathcal{P}(\mathbb{Q})$ durch nach unten abgeschlossene, nach oben offene und nichttriviale Teilmengen der rationalen Zahlen.
\end{itemize}
\end{nice-box}
