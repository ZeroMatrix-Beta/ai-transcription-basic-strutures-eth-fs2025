% ==========================================
% LatexRefinement Step Output: step4-04-21-25-tuesday-all-offset-final.tex
...
% ThinkingLevel: HIGH
% Prompt Tokens: 27'661
% Candidates Tokens: 18'191 (inkl. Thinking Tokens)
% Cached Tokens: 24'544
% Processed on: 2026-07-06 17:56:50
% ==========================================

% Primary Language: German

\lecturechapter{Dienstag}{21. Apr}{21. April 2026}{Kardinalzahlen und der Satz von Cantor-Bernstein-Schröder}
...
\begin{example}[Gleichmächtigkeit von \texorpdfstring{$\omega + \omega$}{omega + omega} und \texorpdfstring{$\omega$}{omega}]\label[example]{ex:omega-plus-omega}
Es gilt:
\[
|\omega + \omega| = |\omega|
\]
...
Die Gleichmächtigkeit verhält sich wie eine Äquivalenzrelation auf der Klasse aller Mengen (für die formalen Axiome der Reflexivität, Symmetrie und Transitivität siehe \cref{def:aequivalenzrelation} in Woche 2). Es gilt für beliebige Mengen $A, B, C$:
\begin{enumerate}[label=\textbf{(\roman*)}]
    ...
    \item \textbf{Transitivität:} Falls $|A| = |B|$ und $|B| = |C|$, so ist $|A| = |C|$ (mittels der Komposition $g \circ f \colon A \times B \to C$).
\end{enumerate}
\end{remark}
\end{math-stroke}
...
% \begin{math-stroke}[Beweis des Cantor-Bernstein-Schröder-Theorems]
% \begin{short-proof}
% Seien $\textcolor{MidnightBlue}{f} \colon A \to B$ und $\textcolor{ForestGreen}{g} \colon B \to A$ injektive Abbildungen.
% Wir definieren rekursiv eine Folge von Teilmengen von $A$:
% \begin{align*}
% E_0 &:= A \setminus \textcolor{ForestGreen}{g}(B) \\
% E_{n+1} &:= \textcolor{ForestGreen}{g}(\textcolor{MidnightBlue}{f}(E_n)) \quad \text{für alle } n \in \omega
% \end{align*}
% Sei $E$ die Vereinigung dieser Mengen:
% \[
% E := \bigcup_{n \in \omega} E_n
...
% \[
% h(x) := \begin{cases}
% \textcolor{MidnightBlue}{f}(x) & \text{falls } x \in E \\
% \textcolor{ForestGreen}{g}^{-1}(x) & \text{falls } x \notin E
% \end{cases}
% \]
...
%   % \begin{ai-tikz-planner-invisible-content}
%   % 1. Background: Verschachtelte Mengen (konzentrische Rechtecke), um die Injektionen ineinander darzustellen.
%   % 2. Midground: Farbliche Trennung der Ringe. Blau für E_n, Grün für den Rest.
%   % 3. Foreground: Farbcodierte Pfeile (f in Blau, g in Grün), die sich überkreuzen und die Abbildungen zwischen den Ringen zeigen.
%   % \end{ai-tikz-planner-invisible-content}
  
%   \colorlet{colF}{MidnightBlue}
%   \colorlet{colG}{ForestGreen}
%   \colorlet{fillE}{MidnightBlue!10}
%   \colorlet{fillRest}{ForestGreen!10}
%   \colorlet{fillCore}{gray!15}

%   % Set A (Verschachtelungen)
%   \filldraw[fill=fillE, draw=colF, thick] (0,0) rectangle (5,7.5);
%   \filldraw[fill=fillRest, draw=colF, thick] (0.3,0.3) rectangle (4.7,6.3);
%   \filldraw[fill=fillE, draw=colF, thick] (0.6,0.6) rectangle (4.4,5.1);
%   \filldraw[fill=fillRest, draw=colF, thick] (0.9,0.9) rectangle (4.1,3.9);
%   \filldraw[fill=fillCore, draw=colF, thick] (1.2,1.2) rectangle (3.8,2.7);

%   \node[colF, font=\Large\bfseries] at (2.5, 7.9) {$A$};
%   \node[colF] at (2.5, 6.9) {$E_0 = A \setminus \textcolor{ForestGreen}{g}(B)$};
%   \node[colG] at (2.5, 5.7) {$\textcolor{ForestGreen}{g}(B \setminus \textcolor{MidnightBlue}{f}(A))$};
%   \node[colF] at (2.5, 4.5) {$E_1 = \textcolor{ForestGreen}{g}(\textcolor{MidnightBlue}{f}(E_0))$};
%   \node[colG] at (2.5, 3.3) {$\textcolor{ForestGreen}{g}(\textcolor{MidnightBlue}{f}(B \setminus \textcolor{MidnightBlue}{f}(A)))$};
%   \node[darkgray] at (2.5, 1.95) {$A_\infty$ (Kern)};

%   % Set B (Verschachtelungen)
%   \filldraw[fill=fillRest, draw=colG, thick] (9,0) rectangle (14,7.5);
%   \filldraw[fill=fillE, draw=colG, thick] (9.3,0.3) rectangle (13.7,6.3);
%   \filldraw[fill=fillRest, draw=colG, thick] (9.6,0.6) rectangle (13.4,5.1);
%   \filldraw[fill=fillE, draw=colG, thick] (9.9,0.9) rectangle (13.1,3.9);
%   \filldraw[fill=fillCore, draw=colG, thick] (10.2,1.2) rectangle (12.8,2.7);

%   \node[colG, font=\Large\bfseries] at (11.5, 7.9) {$B$};
%   \node[colG] at (11.5, 6.9) {$B \setminus \textcolor{MidnightBlue}{f}(A)$};
%   \node[colF] at (11.5, 5.7) {$\textcolor{MidnightBlue}{f}(E_0)$};
%   \node[colG] at (11.5, 4.5) {$\textcolor{MidnightBlue}{f}(\textcolor{ForestGreen}{g}(B \setminus \textcolor{MidnightBlue}{f}(A)))$};
%   \node[colF] at (11.5, 3.3) {$\textcolor{MidnightBlue}{f}(E_1)$};
%   \node[darkgray] at (11.5, 1.95) {$B_\infty$ (Kern)};

%   % Pfeile für f (Blau)
%   \draw[->, colF, thick] (5, 6.9) -- node[above, sloped] {$\textcolor{MidnightBlue}{f}$} (9.3, 5.7);
%   \draw[->, colF, thick] (4.4, 4.5) -- node[above, sloped] {$\textcolor{MidnightBlue}{f}$} (9.9, 3.3);
%   \draw[->, colF, thick] (3.8, 2.1) -- node[above] {$\textcolor{MidnightBlue}{f}$} (10.2, 2.1);

%   % Pfeile für g (Grün)
%   \draw[->, colG, thick] (9, 6.9) -- node[above, sloped] {$\textcolor{ForestGreen}{g}$} (4.7, 5.7);
%   \draw[->, colG, thick] (9.6, 4.5) -- node[above, sloped] {$\textcolor{ForestGreen}{g}$} (4.1, 3.3);
%   \draw[->, colG, thick] (10.2, 1.8) -- node[below] {$\textcolor{ForestGreen}{g}$} (3.8, 1.8);

%   % Legende
%   \node[align=left, anchor=north west] at (0, -0.5) {
%     \textbf{Konstruktion der Bijektion $h$:}\\
%     \tikz[baseline=-0.5ex] \filldraw[fill=fillE, draw=colF, thick] (0,0) rectangle (0.3,0.3); Menge $E$ (blau): Hier wirkt $h = \textcolor{MidnightBlue}{f}$ (Pfeile nach rechts).\\
%     \tikz[baseline=-0.5ex] \filldraw[fill=fillRest, draw=colG, thick] (0,0) rectangle (0.3,0.3); Menge $A \setminus E$ (grün/grau): Hier wirkt $h = \textcolor{ForestGreen}{g}^{-1}$ (Pfeile nach links umgekehrt).
%   };
% \end{tikzpicture}
% \end{center}

% \textbf{1. Wohldefiniertheit von $h$:} \\
% Falls $x \notin E$, so ist insbesondere $x \notin E_0 = A \setminus \textcolor{ForestGreen}{g}(B)$. Daraus folgt $x \in \textcolor{ForestGreen}{g}(B)$. Da $\textcolor{ForestGreen}{g}$ injektiv ist, existiert ein eindeutiges Urbild $\textcolor{ForestGreen}{g}^{-1}(x) \in B$. Somit ist $h$ wohldefiniert.

% \textbf{2. Injektivität von $h$:} \\
% Seien $x_1, x_2 \in A$ mit $h(x_1) = h(x_2)$.
% \begin{itemize}
%     \item \textbf{Fall 1 ($x_1, x_2 \in E$):} Es gilt $\textcolor{MidnightBlue}{f}(x_1) = \textcolor{MidnightBlue}{f}(x_2)$. Da $\textcolor{MidnightBlue}{f}$ injektiv ist, folgt daraus $x_1 = x_2$.
%     \item \textbf{Fall 2 ($x_1, x_2 \notin E$):} Es gilt $\textcolor{ForestGreen}{g}^{-1}(x_1) = \textcolor{ForestGreen}{g}^{-1}(x_2)$. Da $\textcolor{ForestGreen}{g}$ injektiv ist, folgt daraus $x_1 = x_2$.
%     \item \textbf{Fall 3 (Mischfall):} Angenommen, $x_1 \in E$ und $x_2 \notin E$. Dann gilt $\textcolor{MidnightBlue}{f}(x_1) = \textcolor{ForestGreen}{g}^{-1}(x_2)$, woraus $\textcolor{ForestGreen}{g}(\textcolor{MidnightBlue}{f}(x_1)) = x_2$ folgt. Da $x_1 \in E$, existiert ein $n \in \omega$ mit $x_1 \in E_n$. Daraus ergibt sich $x_2 = \textcolor{ForestGreen}{g}(\textcolor{MidnightBlue}{f}(x_1)) \in E_{n+1} \subseteq E$, was im Widerspruch zu $x_2 \notin E$ steht.
% \end{itemize}
% Somit ist $h$ injektiv.
...
% Sei $y \in B$. Wir suchen ein $x \in A$ mit $h(x) = y$.
% \begin{itemize}
%     \item \textbf{Fall 1 ($\textcolor{ForestGreen}{g}(y) \in E$):} Dann existiert ein $n \in \omega$ mit $\textcolor{ForestGreen}{g}(y) \in E_n$. Da $\textcolor{ForestGreen}{g}(y) \in \textcolor{ForestGreen}{g}(B)$, kann $\textcolor{ForestGreen}{g}(y)$ nicht in $E_0$ liegen, also ist $n \ge 1$. Es gilt $\textcolor{ForestGreen}{g}(y) \in E_n = \textcolor{ForestGreen}{g}(\textcolor{MidnightBlue}{f}(E_{n-1}))$. Da $\textcolor{ForestGreen}{g}$ injektiv ist, folgt $y = \textcolor{MidnightBlue}{f}(x)$ für ein $x \in E_{n-1} \subseteq E$. Für dieses $x$ gilt $h(x) = \textcolor{MidnightBlue}{f}(x) = y$.
%     \item \textbf{Fall 2 ($\textcolor{ForestGreen}{g}(y) \notin E$):} Wir wählen $x := \textcolor{ForestGreen}{g}(y) \notin E$. Dann gilt $h(x) = \textcolor{ForestGreen}{g}^{-1}(x) = \textcolor{ForestGreen}{g}^{-1}(\textcolor{ForestGreen}{g}(y)) = y$.
% \end{itemize}
% Somit ist $h$ surjektiv und folglich eine Bijektion. Es gilt $|A| = |B|$.
% \end{short-proof}
% \end{math-stroke}
...
% \begin{didactic-insight}[Die Eleganz des Cantor-Bernstein-Schröder-Theorems]
% Der Beweis des Cantor-Bernstein-Schröder-Theorems ist ein klassisches Beispiel für ein nicht-konstruktives Auswahlverfahren in der Mengenlehre. Anstatt eine explizite Bijektion mühsam von Hand zu definieren --- was bei unendlichen Mengen oft unmöglich ist ---, erlaubt uns der Satz, das Problem auf das Auffinden zweier Injektionen $\textcolor{MidnightBlue}{f} \colon A \to B$ und $\textcolor{ForestGreen}{g} \colon B \to A$ zu reduzieren. 

% Die Grafik veranschaulicht die Genialität des Beweises: Die Funktion $h$ partitioniert die Menge $A$ geschickt in verschachtelte Ringe von Vor- und Nachbildern, um lokale Bijektionen zusammenzuflicken.
% \begin{align*}
%   A &= E \;\dot{\cup}\; (A \setminus E) \\
%   B &= \textcolor{MidnightBlue}{f}(E) \;\dot{\cup}\; (B \setminus \textcolor{MidnightBlue}{f}(E))
% \end{align*}
% Auf $E$ verwenden wir die ``Vorwärtsabbildung'' $\textcolor{MidnightBlue}{f}$, auf dem Komplement $A \setminus E$ die ``Rückwärtsabbildung'' $\textcolor{ForestGreen}{g}^{-1}$.
% \end{didactic-insight}
...
% \begin{math-stroke}[Beweis des Cantor-Bernstein-Schröder-Theorems]
% \begin{short-proof}
% Seien $\textcolor{MidnightBlue}{f} \colon \textcolor{PineGreen}{A} \to \textcolor{Sepia}{B}$ und $\textcolor{BrickRed}{g} \colon \textcolor{Sepia}{B} \to \textcolor{PineGreen}{A}$ injektive Abbildungen.
% Wir definieren rekursiv eine Folge von Teilmengen von $\textcolor{PineGreen}{A}$:
% \begin{align*}
% \textcolor{Plum}{E}_0 &:= \textcolor{PineGreen}{A} \setminus \textcolor{BrickRed}{g}(\textcolor{Sepia}{B}) \\
% \textcolor{Plum}{E}_{n+1} &:= \textcolor{BrickRed}{g}(\textcolor{MidnightBlue}{f}(\textcolor{Plum}{E}_n)) \quad \text{für alle } n \in \omega
% \end{align*}
% Sei $\textcolor{Plum}{E}$ die Vereinigung dieser Mengen:
% \[
% \textcolor{Plum}{E} := \bigcup_{n \in \omega} \textcolor{Plum}{E}_n
% \]
% Wir definieren die Abbildung $h \colon \textcolor{PineGreen}{A} \to \textcolor{Sepia}{B}$ durch:
% \[
% h(x) := \begin{cases}
% \textcolor{MidnightBlue}{f}(x) & \text{falls } x \in \textcolor{Plum}{E} \\
% \textcolor{BrickRed}{g}^{-1}(x) & \text{falls } x \notin \textcolor{Plum}{E}
% \end{cases}
% \]
...
% \begin{tikzpicture}[scale=0.95, >=stealth]
%   % \begin{ai-tikz-planner-invisible-content}
%   % 1. Background: Verschachtelte Mengen. A ist PineGreen, B ist Sepia, E ist Plum.
%   % 2. Farbwahl (dvipsnames): 
%   %    - f = MidnightBlue (Dunkelblau, L~10%)
%   %    - g = BrickRed (Dunkelrot, L~30%)
%   %    - A = PineGreen (Dunkelgrün, L~40%)
%   %    - B = Sepia (Dunkelbraun, L~25%)
%   %    - E = Plum (Dunkellila, L~35%)
%   %    Diese Farben haben unterschiedliche Helligkeitswerte und Farbtöne. Sie sind auf Farbdisplays exzellent unterscheidbar und erzeugen im Schwarz-Weiß-Druck unterschiedliche Graustufen, ohne zu hell zu sein (wie Gelb).
%   % 3. Label-Platzierung: Die Labels für f und g wurden mit pos=0.25 an den Pfeilanfang gesetzt. Da f von links nach rechts und g von rechts nach links verläuft, sind die Labels dadurch maximal voneinander entfernt und überlappen sich nicht.
%   % \end{ai-tikz-planner-invisible-content}
  
%   \colorlet{colA}{PineGreen}
%   \colorlet{colB}{Sepia}
%   \colorlet{colE}{Plum}
%   \colorlet{colF}{MidnightBlue}
%   \colorlet{colG}{BrickRed}
  
%   \colorlet{fillA}{PineGreen!10}
%   \colorlet{fillB}{Sepia!10}
%   \colorlet{fillE}{Plum!10}
%   \colorlet{fillCore}{gray!15}
...
%   \node[colA, font=\Large\bfseries] at (2.5, 7.9) {$\textcolor{PineGreen}{A}$};
%   \node[colE] at (2.5, 6.9) {$\textcolor{Plum}{E}_0 = \textcolor{PineGreen}{A} \setminus \textcolor{BrickRed}{g}(\textcolor{Sepia}{B})$};
%   \node[colA] at (2.5, 5.7) {$\textcolor{BrickRed}{g}(\textcolor{Sepia}{B} \setminus \textcolor{MidnightBlue}{f}(\textcolor{PineGreen}{A}))$};
%   \node[colE] at (2.5, 4.5) {$\textcolor{Plum}{E}_1 = \textcolor{BrickRed}{g}(\textcolor{MidnightBlue}{f}(\textcolor{Plum}{E}_0))$};
%   \node[colA] at (2.5, 3.3) {$\textcolor{BrickRed}{g}(\textcolor{MidnightBlue}{f}(\textcolor{Sepia}{B} \setminus \textcolor{MidnightBlue}{f}(\textcolor{PineGreen}{A})))$};
%   \node[darkgray] at (2.5, 1.95) {$A_\infty$ (Kern)};
...
%   \node[colB, font=\Large\bfseries] at (11.5, 7.9) {$\textcolor{Sepia}{B}$};
%   \node[colB] at (11.5, 6.9) {$\textcolor{Sepia}{B} \setminus \textcolor{MidnightBlue}{f}(\textcolor{PineGreen}{A})$};
%   \node[colE] at (11.5, 5.7) {$\textcolor{MidnightBlue}{f}(\textcolor{Plum}{E}_0)$};
%   \node[colB] at (11.5, 4.5) {$\textcolor{MidnightBlue}{f}(\textcolor{BrickRed}{g}(\textcolor{Sepia}{B} \setminus \textcolor{MidnightBlue}{f}(\textcolor{PineGreen}{A})))$};
%   \node[colE] at (11.5, 3.3) {$\textcolor{MidnightBlue}{f}(\textcolor{Plum}{E}_1)$};
%   \node[darkgray] at (11.5, 1.95) {$B_\infty$ (Kern)};

%   % Pfeile für f (Blau) - pos=0.25 setzt das Label nah an A
%   \draw[->, colF, thick] (5, 6.9) -- node[above, sloped, pos=0.25] {$\textcolor{MidnightBlue}{f}$} (9.3, 5.7);
%   \draw[->, colF, thick] (4.4, 4.5) -- node[above, sloped, pos=0.25] {$\textcolor{MidnightBlue}{f}$} (9.9, 3.3);
%   \draw[->, colF, thick] (3.8, 2.1) -- node[above, pos=0.25] {$\textcolor{MidnightBlue}{f}$} (10.2, 2.1);

%   % Pfeile für g (Rot) - pos=0.25 setzt das Label nah an B (da der Pfeil von B nach A geht)
%   \draw[->, colG, thick] (9, 6.9) -- node[above, sloped, pos=0.25] {$\textcolor{BrickRed}{g}$} (4.7, 5.7);
%   \draw[->, colG, thick] (9.6, 4.5) -- node[above, sloped, pos=0.25] {$\textcolor{BrickRed}{g}$} (4.1, 3.3);
%   \draw[->, colG, thick] (10.2, 1.8) -- node[below, pos=0.25] {$\textcolor{BrickRed}{g}$} (3.8, 1.8);

%   % Legende
%   \node[align=left, anchor=north west] at (0, -0.5) {
%     \textbf{Konstruktion der Bijektion $h$:}\\
%     \tikz[baseline=-0.5ex] \filldraw[fill=fillE, draw=colE, thick] (0,0) rectangle (0.3,0.3); Menge $\textcolor{Plum}{E}$ (lila): Hier wirkt $h = \textcolor{MidnightBlue}{f}$ (Pfeile nach rechts).\\
%     \tikz[baseline=-0.5ex] \filldraw[fill=fillA, draw=colA, thick] (0,0) rectangle (0.3,0.3); Menge $\textcolor{PineGreen}{A} \setminus \textcolor{Plum}{E}$ (grün): Hier wirkt $h = \textcolor{BrickRed}{g}^{-1}$ (Pfeile nach links umgekehrt).
%   };
% \end{tikzpicture}
% \end{center}

% \textbf{1. Wohldefiniertheit von $h$:} \\
% Falls $x \notin \textcolor{Plum}{E}$, so ist insbesondere $x \notin \textcolor{Plum}{E}_0 = \textcolor{PineGreen}{A} \setminus \textcolor{BrickRed}{g}(\textcolor{Sepia}{B})$. Daraus folgt $x \in \textcolor{BrickRed}{g}(\textcolor{Sepia}{B})$. Da $\textcolor{BrickRed}{g}$ injektiv ist, existiert ein eindeutiges Urbild $\textcolor{BrickRed}{g}^{-1}(x) \in \textcolor{Sepia}{B}$. Somit ist $h$ wohldefiniert.

% \textbf{2. Injektivität von $h$:} \\
% Seien $x_1, x_2 \in \textcolor{PineGreen}{A}$ mit $h(x_1) = h(x_2)$.
% \begin{itemize}
%     \item \textbf{Fall 1 ($x_1, x_2 \in \textcolor{Plum}{E}$):} Es gilt $\textcolor{MidnightBlue}{f}(x_1) = \textcolor{MidnightBlue}{f}(x_2)$. Da $\textcolor{MidnightBlue}{f}$ injektiv ist, folgt daraus $x_1 = x_2$.
%     \item \textbf{Fall 2 ($x_1, x_2 \notin \textcolor{Plum}{E}$):} Es gilt $\textcolor{BrickRed}{g}^{-1}(x_1) = \textcolor{BrickRed}{g}^{-1}(x_2)$. Da $\textcolor{BrickRed}{g}$ injektiv ist, folgt daraus $x_1 = x_2$.
%     \item \textbf{Fall 3 (Mischfall):} Angenommen, $x_1 \in \textcolor{Plum}{E}$ und $x_2 \notin \textcolor{Plum}{E}$. Dann gilt $\textcolor{MidnightBlue}{f}(x_1) = \textcolor{BrickRed}{g}^{-1}(x_2)$. Da $x_2 \notin \textcolor{Plum}{E}$, liegt $x_2$ im Bild von $\textcolor{BrickRed}{g}$, weshalb die Anwendung von $\textcolor{BrickRed}{g}$ auf beide Seiten wohldefiniert ist und $\textcolor{BrickRed}{g}(\textcolor{MidnightBlue}{f}(x_1)) = x_2$ liefert. Da $x_1 \in \textcolor{Plum}{E}$, existiert ein $n \in \omega$ mit $x_1 \in \textcolor{Plum}{E}_n$. Daraus ergibt sich $x_2 = \textcolor{BrickRed}{g}(\textcolor{MidnightBlue}{f}(x_1)) \in \textcolor{Plum}{E}_{n+1} \subseteq \textcolor{Plum}{E}$, was im Widerspruch zu $x_2 \notin \textcolor{Plum}{E}$ steht.
% \end{itemize}
% Somit ist $h$ injektiv.

% \textbf{3. Surjektivität von $h$:} \\
% Sei $y \in \textcolor{Sepia}{B}$. Wir suchen ein $x \in \textcolor{PineGreen}{A}$ mit $h(x) = y$.
% \begin{itemize}
%     \item \textbf{Fall 1 ($\textcolor{BrickRed}{g}(y) \in \textcolor{Plum}{E}$):} Dann existiert ein $n \in \omega$ mit $\textcolor{BrickRed}{g}(y) \in \textcolor{Plum}{E}_n$. Da $\textcolor{BrickRed}{g}(y) \in \textcolor{BrickRed}{g}(\textcolor{Sepia}{B})$, kann $\textcolor{BrickRed}{g}(y)$ nicht in $\textcolor{Plum}{E}_0$ liegen, also ist $n \ge 1$. Es gilt $\textcolor{BrickRed}{g}(y) \in \textcolor{Plum}{E}_n = \textcolor{BrickRed}{g}(\textcolor{MidnightBlue}{f}(\textcolor{Plum}{E}_{n-1}))$. Da $\textcolor{BrickRed}{g}$ injektiv ist, folgt $y = \textcolor{MidnightBlue}{f}(x)$ für ein $x \in \textcolor{Plum}{E}_{n-1} \subseteq \textcolor{Plum}{E}$. Für dieses $x$ gilt $h(x) = \textcolor{MidnightBlue}{f}(x) = y$.
%     \item \textbf{Fall 2 ($\textcolor{BrickRed}{g}(y) \notin \textcolor{Plum}{E}$):} Wir wählen $x := \textcolor{BrickRed}{g}(y) \notin \textcolor{Plum}{E}$. Dann gilt $h(x) = \textcolor{BrickRed}{g}^{-1}(x) = \textcolor{BrickRed}{g}^{-1}(\textcolor{BrickRed}{g}(y)) = y$.
% \end{itemize}
% Somit ist $h$ surjektiv und folglich eine Bijektion. Es gilt $|\textcolor{PineGreen}{A}| = |\textcolor{Sepia}{B}|$.
% \end{short-proof}
% \end{math-stroke}
...
% Die Grafik veranschaulicht die Genialität dieses konstruktiven Ansatzes: Die Funktion $h$ partitioniert die Menge $\textcolor{PineGreen}{A}$ geschickt in verschachtelte Ringe von Vor- und Nachbildern, um lokale Teilstücke von Bijektionen nahtlos zusammenzuflicken:
% \begin{align*}
%   \textcolor{PineGreen}{A} &= \textcolor{Plum}{E} \;\dot{\cup}\; (\textcolor{PineGreen}{A} \setminus \textcolor{Plum}{E}) \\
%   \textcolor{Sepia}{B} &= \textcolor{MidnightBlue}{f}(\textcolor{Plum}{E}) \;\dot{\cup}\; (\textcolor{Sepia}{B} \setminus \textcolor{MidnightBlue}{f}(\textcolor{Plum}{E}))
% \end{align*}
% Auf der lila Teilmenge $\textcolor{Plum}{E}$ verwenden wir die direkte „Vorwärtsabbildung“ $\textcolor{MidnightBlue}{f}$, während wir auf dem grünen Komplement $\textcolor{PineGreen}{A} \setminus \textcolor{Plum}{E}$ die Umkehrung der „Rückwärtsabbildung“ $\textcolor{BrickRed}{g}^{-1}$ nutzen, um den Rest der Mengen perfekt auszufüllen.
% \end{didactic-insight}
...
\begin{tikzpicture}[scale=0.95, >=stealth]
  ...
  \node[align=left, anchor=north west] at (0, -0.5) {
    ...
    \tikz[baseline=-0.5ex] \filldraw[fill=fillA, draw=colA, thick] (0,0) rectangle (0.3,0.3); Menge $\textcolor{ForestGreen}{A} \setminus \textcolor{Fuchsia}{E}$ (grün): Hier wirkt $h = \textcolor{BurntOrange}{g}^{-1}$ (Pfeile nach links umgekehrt).
  };
\end{tikzpicture}
\end{center}
...
\end{align*}
Auf der lila/pinken Teilmenge $\textcolor{Fuchsia}{E}$ verwenden wir die direkte „Vorwärtsabbildung“ $\textcolor{RoyalBlue}{f}$, während wir auf dem grünen Komplement $\textcolor{ForestGreen}{A} \setminus \textcolor{Fuchsia}{E}$ die Umkehrung der „Rückwärtsabbildung“ $\textcolor{BurntOrange}{g}^{-1}$ nutzen, um den Rest der Mengen perfekt auszufüllen.
\end{didactic-insight}
...
Eine Ordinalzahl $\kappa \in \Omega$ heißt \newterm{Kardinalzahl}, falls für alle Ordinalzahlen $\alpha < \kappa$ gilt:
...
Gemäß dem Wohlordnungsprinzip (\textbf{WOP}) existiert für jede Menge $M$ eine Ordinalzahl $\alpha \in \Omega$ mit $|M| = |\alpha|$.
...
Wir können auch zeigen, dass jede unendliche Kardinalzahl eine Limes-Ordinalzahl ist --- das gilt natürlich nicht für die endlichen Kardinalzahlen, aber für unendliche Kardinalzahlen gilt das immer. Und wir schreiben für diese unendlichen Kardinalzahlen oft $\aleph_\alpha$ oder $\omega_\alpha$.
\end{spoken-clean}
...
\begin{math-stroke}
\begin{theorem}[Satz von Cantor]\label[theorem]{thm:cantor-proof}
Für jede Menge $M$ gilt:
\begin{equation}\label{eq:cantor-thm}
|M| < |\mathcal{P}(M)|
\end{equation}
\end{theorem}

\begin{short-proof}[Beweis für $M = \emptyset$]
...
\begin{itemize}
    ...
    \item \textbf{Paul Cohen (1963):} Zeigte die relative Konsistenz von $\text{ZFC} + \neg\text{CH}$ (mittels der Methode des Forcings). $\text{CH}$ kann in $\text{ZFC}$ nicht beweisen werden.
\end{itemize}
...
\]
Der Satz von Cantor (Theorem \ref{thm:cantor-proof}) lässt sich in dieser Notation wie folgt formulieren:
